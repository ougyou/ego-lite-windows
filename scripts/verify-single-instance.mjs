...
const TAB_SCRIPT = `
const task = await taskSpaces.useOrCreate('single-instance')
...
const REUSE_SCRIPT = `
const task = await taskSpaces.useOrCreate('single-instance')
console.log('TABS=' + (await browser.listTabs()).length)
`;
...
try {
  ...
  // S1: cold-start direct-to-page, then immediately re-run.
  const s1 = await runEgo(
    ["--url", "https://example.com", "--headless", "nodejs"],
    TAB_SCRIPT,
  );
  check("S1 cold-start direct-to-page runs", s1.code === 0, `exit=${s1.code}`);
  // Assert via listTabs (the tab's real URL). page.info() can resolve to the
  // harness's CDP anchor (about:blank) — a known quirk, not a blank first tab.
  ...
  const s1b = await runEgo(["--headless", "nodejs"], REUSE_SCRIPT);
  ...
  const [p1, p2] = await Promise.all([
    runEgo(["--headless", "nodejs"], TAB_SCRIPT),
    runEgo(["--headless", "nodejs"], REUSE_SCRIPT),
  ]);
  ...
  if (VISIBLE) {
    ...
    const task = await runEgo(["--url", "https://example.com", "nodejs"], TAB_SCRIPT);
