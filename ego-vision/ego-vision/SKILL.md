...
If `ego-vision` is not on PATH, run `node <repo>\ego-vision\cli\ego-vision.mjs ocr ...`.

## 单次隔离与后置清理（IMPORTANT）

每次操作**只处理本次携带的图片**，绝不复用旧图片/旧结果：

- **只 OCR 本次操作提供的图片**（粘贴的图 / 明确指定的路径）。禁止去历史消息、`testdata/`、`%TEMP%` 或任何旧位置"找之前的图"——旧图一律不理会、不复用。
- **粘贴/附件图（无磁盘路径）**：先落盘为**唯一命名**的临时文件（如 `$env:TEMP/ego-vision-<时间戳>-<随机>.png`），再对它 OCR。**禁止用固定文件名**（如 `C:/tmp/shot.png`、`card1-ocr.json`）——固定名会被旧文件覆盖/读到旧内容。
- **后置清理**：OCR 完成后**删除本次创建的临时文件**（保持区域干净）；只删自己创建的，不删他人/历史的文件。
- **找不到本次图片**：明确询问用户图片在哪里，不要擅自翻旧图或猜。
