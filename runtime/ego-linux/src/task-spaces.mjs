...
export function createTaskSpacesApi(cdp) {
  ...
  /**
   * Merge a space's cookies back into the default jar before its context dies.
   *
   * A space's browser context is an in-memory cookie jar: logins made inside it
   * live nowhere on disk. Disposing the context (space close) or losing it
   * (browser restart) therefore used to drop the session silently — a login
   * done in a space never survived to the next session. Reflowing into the
   * default jar first is what makes a space's login persistent: it lands in the
   * profile layer, survives restarts, and the next space seeds itself from it.
   */
  async function reflowSpaceCookies(browserContextId) {
    try {
      const { cookies = [] } = await cdp.call("Storage.getCookies", {
        browserContextId,
      });
      if (cookies.length) {
        // No browserContextId on the set => the default jar, the persistent one.
        await cdp.call("Storage.setCookies", { cookies });
      }
    } catch {
      // Best-effort: a space that fails to flush still closes cleanly.
    }
  }

  async function disposeContext(browserContextId) {
    // Persist the space's logins into the default jar before its jar dies.
    await reflowSpaceCookies(browserContextId);
    await cdp
  ...
  /**
   * Does a target's url equal the requested start url, ignoring a trailing
   * slash (Chrome normalises `https://example.com` to `https://example.com/`).
   */
  function startUrlMatches(targetUrl, startUrl) {
    if (!targetUrl) return false;
    if (targetUrl === startUrl) return true;
    try {
      ...
      return targetUrl.startsWith(startUrl);
    }
  }

  /**
   * The cold-start page to adopt, when a direct-to-page launch made one.
   *
   * With EGO_LINUX_START_URL set, Chrome opens holding the target URL in the
   * default context — before any space exists. Adopting that tab as the first
   * space's anchor means the startup page and the space's own blank tab do not
   * exist side by side (the "it opened another browser" symptom), and because
   * the tab shares the default jar, a --url first run inherits and persists
   * logins directly.
   *
   * The default context is not recognisable by an empty browserContextId:
   * stock Chrome reports it with a real (non-empty) id too. What *is* unique
   * about it is that Target.createBrowserContext never made it — so a context
   * that is not in Target.getBrowserContexts() is the default one.
   *
   * Only engaged for explicit --url cold starts; otherwise null, and spaces
   * keep their own isolated, seeded context as before.
   */
  async function adoptStartupTarget() {
    if (!process.env.EGO_LINUX_START_URL) return null;
    try {
      const { targetInfos = [] } = await cdp.call("Target.getTargets");
      ...
      const startUrl = process.env.EGO_LINUX_START_URL;
      return (
        targetInfos.find(
          (target) =>
            target.type === "page" &&
            ...
            // Default context = not one of ours (empty id, or a real id that
            // Target.createBrowserContext never produced).
            (!target.browserContextId || !created.has(target.browserContextId)) &&
            startUrlMatches(target.url, startUrl),
        ) || null
      );
    } catch {
      return null;
  ...
  return {
    ...
    async createTaskSpace(name) {
      ...
      // First-run direct-to-page: adopt the browser's cold-start tab as this
      // space's anchor instead of opening a second, blank one. The adopted tab
      // lives in the default jar (no context), which is exactly what makes a
      // --url first run persistent and free of an extra blank window.
      const startupTarget = await adoptStartupTarget();
      let browserContextId;
      let targetId;
      if (startupTarget) {
        browserContextId = null;
        targetId = startupTarget.targetId;
      } else {
        browserContextId = await createSeededContext();
        try {
