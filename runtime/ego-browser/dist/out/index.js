...
function relativeSitePath(siteDir, manifestPath, label) {
    ...
    // Windows compatibility: path.resolve returns backslash-separated paths,
    // but the original check joined with a forward slash, so every relative
    // tool path failed the "inside the site skill directory" test on Windows.
    // Use the separator actually in use.
    const sep = siteRoot.includes("\\") ? "\\" : "/";
    const prefix = `${siteRoot}${sep}`;
    if (resolved !== siteRoot && !resolved.startsWith(prefix)) {
